...
- If you need to update the commit message
  ...
  - Be mindful: Only amend commits that haven’t been pushed yet
...
1. **Fix Files**: Stage any new or modified files for commit  
   - `git add <file>`
2. **Amend the Commit**:  
   - `git commit --amend`
   - Updates the last commit with the staged files
3. **Modify Commit Message** (optional):  
   - Git editor opens to update commit message
   - If not modifying, the last message will be used again
...
1. You commit a file, then realize you forgot to include another file.
2. Run:
   ```bash
